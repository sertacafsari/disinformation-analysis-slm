...
from transformers import PhiForSequenceClassification, DataCollatorWithPadding, PhiConfig
...
best_model_save_path = f"./data/models/phi_liar2/best.pt" #TODO: Make it dynamic
save_raw_data_path = f"../data/datasets/liar2/"
save_tokenized_data_path = f"../data/tokenized_datasets/liar2_phi/"
...
tokenized_dataset, tokenizer = datasetLoader.tokenizeDatasetAndSave(model_name=phi,
...
# Set a Config for phi Model
conf = PhiConfig.from_pretrained(phi)
...
for lr in learning_rates:
    ...
    model = PhiForSequenceClassification.from_pretrained(phi, config=conf)
    ...
    wandb.init(
        ...
        name=f"phi-{batch_size}-{lr}-last-run",
        config={
            "model": phi,
            "dataset": "LIAR2",
            "batch_size": batch_size,
    ...
    best_model_save_path = f"./data/models/phi_liar2/phi_{batch_size}_{lr}.pt"
