...
from transformers import DataCollatorWithPadding, AutoModelForSequenceClassification, AutoConfig
...
best_model_save_path = f"./data/models/smol_liar2/best.pt" #TODO: Make it dynamic
save_raw_data_path = f"../data/datasets/liar2/"
save_tokenized_data_path = f"../data/tokenized_datasets/liar2_smol/"
...
tokenized_dataset, tokenizer = datasetLoader.tokenizeDatasetAndSave(model_name=smol,
...
# Set a Config for smol Model
conf = AutoConfig.from_pretrained(smol)
...
for lr in learning_rates:
    ...
    model = AutoModelForSequenceClassification.from_pretrained(smol, config=conf, ignore_mismatched_sizes=True)
    ...
    wandb.init(
        ...
        name=f"smol-{batch_size}-{lr}-last-run",
        config={
            "model": smol,
            "dataset": "LIAR2",
            "batch_size": batch_size,
    ...
    best_model_save_path = f"./data/models/smol_liar2/smol_{batch_size}_{lr}.pt"
